...
DEPE.randomNum = (min, max) => (min && max)? Math.floor(Math.random() * (max - min + 1) ) + min: DEPE.randomNum(5,30);
...
DEPE.getMonth = (idx, key) => configs.months[parseInt(idx)][key];

DEPE.unique = (data, key) => [...new Set(data.map(dt => dt[key]))];